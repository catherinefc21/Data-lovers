...
export const filterProducer = (data, respuestaUsuario) => {
  const filterProducer = data.filter(
    (productor) => productor.producer === respuestaUsuario
  );
  return filterProducer;
};

export const filterDirector = (data, respuestaUsuario) => {
  const filterDirector = data.filter(
    (director) => director.director === respuestaUsuario
  );
  return filterDirector;
};
export const selectMovie = (data, respuestaUsuario) => {
  const selectMovie = data.find(
    (pelicula) => pelicula.title === respuestaUsuario
  );
  return selectMovie;
};
