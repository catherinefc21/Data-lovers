...
<div class="welcome">
<div class="textWelcome"><h2>Bienvenido(a)<br></h2><h3>¡Revisa nuestro menú y descubre todo sobre tus películas favoritas!</h3><br></div>
<br><img class="inicioImg"src="img/portada 3.png" alt="">
</div>
</div>`;
...
todas.addEventListener("click", function () {
  ...
  movies.forEach((movie) => {
    document.getElementById(
      "contenedor2"
    ).innerHTML += `<section moviename="${movie.title}"> <br><img moviename="${movie.title}" src="${movie.poster}">  <br>${movie.rt_score} ⭐ <br>  ${movie.title} </section>`;
  });
  ...
  typeOrder.addEventListener("change", () => {
    ...
    ordenarPeliculas1.forEach((movie) => {
      document.getElementById(
        "contenedor2"
      ).innerHTML += `<section moviename1="${movie.title}"> <br><img moviename1="${movie.title}" src="${movie.poster}">  <br>${movie.rt_score} ⭐ <br>  ${movie.title} </section>`;
    });
  });
  ...
  document
    ...
    .addEventListener("click", function (e) {
      const movieTarjet =
        e.target.getAttribute("moviename") ||
        e.target.getAttribute("moviename1");
      if (movieTarjet) {
        contenedor.innerHTML = "";
        ...
        <div class=picture><img src="${selectMovieTarjet.poster}"></div>
          ...
          selectMovieTarjet.people.forEach((personaje) => {
            document.getElementById(
              "characters"
            ).innerHTML += `<div class= "characterName"><br><div class=nameImg><img src="${personaje.img} "></div><br><div class=name><b>${personaje.name}</b></div></div>`;
          });
          ...
          document.getElementById(
            "caracter"
          ).innerHTML += `<div class="promedio"><b>Promedio edad:</b> ${agePromedio} años.</div>`;
        }
      }
    });
});
